...
class NeedCrypto {
    ...
    static boolean needDecrypt(MethodParameter parameter) {
        boolean encrypt = false;
        boolean classPresentAnno  = parameter.getContainingClass().isAnnotationPresent(com.springCloud.customerFestival.EncryDecryUtil.DecryptRequest.class);
        boolean methodPresentAnno = parameter.getMethod().isAnnotationPresent(com.springCloud.customerFestival.EncryDecryUtil.DecryptRequest.class);

        if(classPresentAnno){
            //类上标注的是否需要解密
            encrypt = parameter.getContainingClass().getAnnotation(com.springCloud.customerFestival.EncryDecryUtil.DecryptRequest.class).value();
            //类不加密，所有都不加密
            if(!encrypt){
                return false;
            }
        ...
        if(methodPresentAnno){
            //方法上标注的是否需要解密
            encrypt = parameter.getMethod().getAnnotation(com.springCloud.customerFestival.EncryDecryUtil.DecryptRequest.class).value();
        }
        return encrypt;
    }
}
